...
fig1dat <- data_longitudinal %>%
  ...
  pivot_longer(!stress, values_to = "strain", names_to = "group") %>% print()

fig1dat %>%
  ggplot(aes(x=strain, y=stress, col=group)) + geom_line() + ggtitle("Figure 1: Stress-Strain Curve for Longitudinal Section of Intestine") + ylab("Strain (Pa)") + xlab("Stress") + labs(col="Phase") + theme_minimal()
...
fig2dat %>%
  ggplot(aes(x=strain, y=stress, col=group)) + geom_line() + ggtitle("Figure 3: Stress-Strain Curve for Circumferential Section of Intestine") + ylab("Strain (Pa)") + xlab("Stress") + labs(col="Phase") + theme_minimal()
